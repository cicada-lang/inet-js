...
export function checkRuleNodeOrder(
  ...
  if (!first.input.some((port) => port.isPrincipal)) {
    throw new Error(
      [
        `[checkRuleNodeOrder] The first node of a rule must have its principal port in the input.`,
        ``,
        ` first node: ${firstName}`,
        ` second node: ${secondName}`,
      ].join("\n"),
  ...
  if (!second.output.some((port) => port.isPrincipal)) {
    throw new Error(
      [
        `[checkRuleNodeOrder] The second node of a rule must have its principal port in the output.`,
        ``,
        ` first node: ${firstName}`,
        ` second node: ${secondName}`,
      ].join("\n"),
